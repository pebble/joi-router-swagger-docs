 ...
 * @param {object} doc - base Swagger doc
 * @param {object} doc.info - Swagger info object
 * @param {string} doc.info.title - API/Application title
 * @param {string} doc.info.version - API version
 * @param {Router|Route[]} - `joi-router` router or routes array
 * @param {object} [options]
 * @param {Function} [options.warnFunc] - function to call when
 *   there's a problem building the Swagger doc; defaults to
 *   `console.warn`. Pass a falsy value to suppress.
 */
exports.generateSwaggerDocs = function(doc, routesOrRouter, options) {
  options = Object.assign({
    warnFunc: console.warn
  }, options);

  assert(doc.info, 'doc info parameter missing');
  assert(doc.info.title, 'doc info.title parameter missing');
  assert(doc.info.version, 'doc info.version parameter missing');
  ...
  if (routesOrRouter.routes && routesOrRouter.use) {
    routes = routesOrRouter.routes;
  } else {
    routes = routesOrRouter;
  }

  if (!Array.isArray(routes)) {
    throw new Error('bad routes: not an array');
  }

  doc.paths = exports.routesToSwaggerPaths(routes, options);

  return doc;
};

function mergeSwaggerPaths(paths, newPaths, options) {
...
exports.routesToSwaggerPaths = function routesToSwaggerPaths(routes, options) {
  ...
  routes.forEach(function(route) {
    let routePaths = exports.routeToSwaggerPaths(route);

    mergeSwaggerPaths(paths, routePaths, options);
  });
...
function hashAPIName(name, version) {
  return `${name}\0${version}`;
}

function getOrCreateAPI(name, version) {
  assert(name, 'missing API name');
  assert(version, 'missing API version');

  let key = `${name}\0${version}`;

  APIs[key] = APIs[key] || {routes: []};

  return APIs[key];
}

exports.addRouterToAPI = function(router, apiName, apiVersion) {
  let api = getOrCreateAPI(apiName, apiVersion);

  api.routes = api.routes.concat(router.routes);
};

exports.getSwaggerHandler = function(apiName, apiVersion, options) {
  let api = getOrCreateAPI(apiName, apiVersion);

  return function* swaggerDocHandler() {
    const ctx = this; // make koa 2.x migration easier

    let doc = exports.generateSwaggerDocs({
      info: {
        title: apiName,
        version: apiVersion
      }
